...
def create_pgp_key(email):
	...
	input_data = gpg.gen_key_input(name_email=email, name_real=email, name_comment='Generated by GoDark.ca', key_type="RSA", key_length=2048)
	key = gpg.gen_key(input_data)
	time_stop = time.time()
	time_total = int(time_stop - time_start)
	logging.debug('Generated PGP key for ' + email + ' in ' + str(time_total) + ' seconds')

	## upload key
	keyid = check_pgp_key(email, True)
	for server in key_servers:
		server_url = key_servers[server]['url'].split("/")[2]
		result = gpg.send_keys(server_url, keyid)
...
def check_pgp_key(email, keyid=False):
...
def send_email(receiver, sender, subject, body, bm_id):
	...
	if not check_pgp_key(sender, True):
		create_pgp_key(sender)

	## sign message
	user_keyid = check_pgp_key(sender, True)
	if user_keyid:
		signed_text = gpg.sign(body, keyid=user_keyid)
		if signed_text:
			body = str(signed_text)
		else:
			logging.error('Found the user\'s keyid, but could not sign message from ' + sender)
	else:
		logging.error('Could not find user\'s keyid: ' + sender)

	## search for recipient PGP key
	if find_pgp_key(receiver):
		fingerprint = check_pgp_key(receiver)

		## if the key was imported fine, encrypt the message
		if fingerprint:

			## only encrypt if the operation was successful
			enc_body = encrypt_text(body, fingerprint)
			if enc_body:
				body = enc_body

			## delete PGP key for privacy reasons
			result = gpg.delete_keys(fingerprint)
			logging.debug('Purging PGP key: ' + str(result))
	...
	for k in dir:
		...
		if not msg_body:
			for part in msg_tmp.walk():
				...
				if part and part.get_content_type() == 'application/octet-stream':
					part_str = part.get_payload(decode=1)

					## if we see the pgp header, decrypt
					if 'BEGIN PGP MESSAGE' in part_str:
						decrypted_data = gpg.decrypt(part_str)
						...
						decrypted_data = email.message_from_string(decrypted_data.data)

						## extract decrypted data
						for decrypted_part in decrypted_data.walk():
							if decrypted_part and decrypted_part.get_content_type() == 'text/plain':
								decrypted_str = decrypted_part.get_payload(decode=1)
								if decrypted_part.get_content_charset():
									msg_body = msg_body + decrypted_str.decode(decrypted_part.get_content_charset())
								else:
									msg_body = msg_body + decrypted_str
